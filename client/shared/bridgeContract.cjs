  ...
  tasks: {
    ...
    startOutlineGeneration: implementedBridgeContract({
      ...
      input: [contractArg('payload', 'StartOutlineGenerationInput', {
        properties: {
          reference_knowledge_document_ids: { type: 'string[]', required: true },
          outline_expansion_mode: { type: 'OutlineExpansionMode', required: true, enum: contractEnums.OutlineExpansionMode },
          word_control_options: { type: 'OutlineWordControlOptions', required: true },
        },
      })],
      output: 'BackgroundTaskState',
      errors: ['TASK_INVALID_INPUT', 'TASK_CONFLICT', 'TASK_INPUT_CHANGED', 'TASK_INTERRUPTED_BY_RESTART'],
    }),
    startGlobalFactsGeneration: implementedBridgeContract({
      owner: 'workflow',
      workPackage: 'WR-03',
      contractRef: 'tasks.startGlobalFactsGeneration',
      input: [contractArg('payload', 'StartGlobalFactsGenerationInput', { required: false })],
      output: 'BackgroundTaskState',
      errors: ['TASK_INVALID_INPUT', 'TASK_CONFLICT', 'TASK_INPUT_CHANGED', 'TASK_INTERRUPTED_BY_RESTART'],
    }),
    startContentGeneration: implementedBridgeContract({
      owner: 'workflow',
      workPackage: 'WR-03',
      contractRef: 'tasks.startContentGeneration',
      input: [contractArg('payload', 'StartContentGenerationInput')],
      output: 'BackgroundTaskState',
      errors: ['TASK_INVALID_INPUT', 'TASK_CONFLICT', 'TASK_INPUT_CHANGED', 'TASK_INTERRUPTED_BY_RESTART'],
    }),
    pauseContentGeneration: implementedBridgeContract({
      owner: 'workflow',
      workPackage: 'WR-03',
      contractRef: 'tasks.pauseContentGeneration',
      input: [contractArg('payload', 'PauseContentGenerationInput', { required: false })],
      output: 'BackgroundTaskState',
      errors: ['TASK_INVALID_INPUT'],
    }),
