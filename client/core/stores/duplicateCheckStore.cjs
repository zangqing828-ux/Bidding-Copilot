...
const { deleteImportedImageBatches } = require('../workspaceCleanup.cjs');
...
function createDuplicateCheckStore({ db, workspaceRoot }) {
  const wp = resolveWorkspacePaths(workspaceRoot);
  const duplicateCheckDir = wp.duplicateCheckDir;
  const contentDir = wp.duplicateCheckContentDir;
  ...
  function clearDuplicateCheck() {
    ...
    deleteImportedImageBatches(workspaceRoot, 'duplicate-check-content');
    ensureDirectories();
