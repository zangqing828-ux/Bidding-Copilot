...
function extractJsonObject(content) {
  ...
  if (!text) throw new Error('图片编排结果为空');
  ...
  try {
    ...
    if (start < 0) throw new Error('图片编排结果不是 JSON 对象');
    let depth = 0;
    let inString = false;
    let escaped = false;
...
function resolveIllustrationPlan(content, context) {
  ...
  if (!parsed || typeof parsed !== 'object' || !Array.isArray(parsed.items)) {
    throw new Error('图片编排结果缺少 items 数组');
  }
  const extraRootFields = Object.keys(parsed).filter((key) => key !== 'items');
  if (extraRootFields.length) throw new Error(`图片编排结果包含多余字段：${extraRootFields.join(', ')}`);
