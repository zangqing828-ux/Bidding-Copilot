...
function validateStoredOutlineDto(rawOutline, workflowKind) {
  if (!rawOutline) {
    return;
  }
  try {
...
async function runFinalOutlineGate({
  ...
  const finalReview = await reviewFinalOutline(aiService, context, log, signal);
  if (!finalReview.passed) {
    throw createTaskOutputInvalidError(
      `最终目录审核未通过${formatSuggestions(finalReview.suggestions)}`,
    );
  }
...
async function alignedWorkflow(aiService, payload, log, signal) {
  log('开始提取技术评分大类。', OUTLINE_PROGRESS.requirementExtractionStart);
  const groups = await extractRequirementGroups(aiService, payload, undefined, log, signal);
  log('技术评分大类提取完成，正在构建一级目录。', OUTLINE_PROGRESS.requirementExtractionEnd);
  const outline = await buildAligned(
    aiService,
    payload,
    groups,
    undefined,
    log,
    { start: OUTLINE_PROGRESS.mainChildrenStart, end: OUTLINE_PROGRESS.mainChildrenEnd },
    signal,
  );
...
async function adjustOutlineForWordControl({
  ...
  let current = outline;
  let attempts = 0;
  let leafCount = countOutlineLeafItems(current?.outline || []);

  while (attempts < MAX_WORD_ADJUSTMENT_ATTEMPTS
    && getLeafCountDistance(leafCount, wordControl.minimumLeafCount, wordControl.maximumLeafCount) > 0) {
    attempts += 1;
    onStats({ phase: 'word-adjusting', attempts, leafCount });
    log(`开始第 ${attempts}/${MAX_WORD_ADJUSTMENT_ATTEMPTS} 次目录叶子数量调整。`, OUTLINE_PROGRESS.wordAdjustmentStart);
    const context = {
      payload,
      outline: current,
      groups,
    ...
    current = await collectJson(aiService, {
      messages: [{
        role: 'user',
        content: `请在保持一级目录和评分项映射不变的前提下调整目录叶子数量。目标范围：${wordControl.minimumLeafCount ?? 0}-${wordControl.maximumLeafCount ?? '不限'}。只返回完整 JSON 目录。\n\n当前目录：\n${JSON.stringify(current, null, 2)}`,
      }],
      temperature: 0.2,
      normalizer: (value) => normalizeWordAdjustedOutlineResult(value, context),
      progressLabel: '目录字数调整',
      failureMessage: '模型返回的目录字数调整结果无效',
    });
    leafCount = countOutlineLeafItems(current?.outline || []);
  }

  const warning = getLeafCountDistance(leafCount, wordControl.minimumLeafCount, wordControl.maximumLeafCount) > 0
    ? OUTLINE_LEAF_COUNT_WARNING
    : '';
  return {
    outline: current,
    leafCount,
    attempts,
    warning,
    warningKind: warning ? 'leaf-count' : '',
  };
}
...
async function runOutlineGenerationTask({
  ...
  validateStoredOutlineDto(storedPlan.outlineData?.outline, isExpansionWorkflow ? 'existing-plan-expansion' : 'technical-plan');
  ...
  if (isExpansionWorkflow) {
    ...
    oldOutline = await extractOriginalOutline(aiService, workspaceStore, originalPlanMarkdown, log, signal);
  }
  ...
  if (isExpansionWorkflow) {
    ...
    const alignedResult = await alignedWorkflow(aiService, taskPayload, log, signal);
